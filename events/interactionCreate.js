...
function buildCaoNutEmbed(gameData) {
      ...
      if (interaction.isButton()) {
        ...
        if (buttonId === 'caonut_join') {
          ...
          if (!gameData) {
            return await safeRespond(() =>
              interaction.reply({
                content: '❌ Ván Cào Nút này đã kết thúc!',
                ephemeral: true
              })
            );
          }

          if (gameData.started || gameData.ended || gameData.handData) {
            return await safeRespond(() =>
              interaction.reply({
                content: '❌ Ván Cào Nút đã bắt đầu, không thể tham gia nữa!',
                ephemeral: true
              })
            );
          }
          ...
          if (gameData.players.has(userId)) {
            return await safeRespond(() =>
              interaction.reply({
                content: '⚠️ Bạn đã tham gia ván Cào Nút này rồi!',
                ephemeral: true
              })
            );
          }
          ...
          try {
            ...
            await interaction.message.edit({
              embeds: [buildCaoNutEmbed(gameData)],
              components: [buildCaoNutJoinRow()]
            });
          } catch (err) {
            ...
            console.error('❌ Lỗi cập nhật ván Cào Nút:', err);
            return;
          }

          console.log(`🃏 ${interaction.user.username} đã tham gia Cào Nút ${gameMsgId}`);
          return;
        }
        ...
        // CÀO NÚT - MỞ LÁ 3
        // =================================================
        if (buttonId.startsWith('caonut_reveal_')) {
          ...
          if (userId !== targetUserId) {
            return await safeRespond(() =>
              interaction.reply({
                content: '❌ Đây không phải lá bài của bạn!',
                ephemeral: true
              })
            );
          }
          ...
          if (!gameData || !gameData.handData || !gameData.dmData) {
            return await safeRespond(() =>
              interaction.reply({
                content: '❌ Ván Cào Nút này đã kết thúc hoặc chưa phát bài!',
                ephemeral: true
              })
            );
          }
          ...
          if (dmData.revealed) {
            return await safeRespond(() =>
              interaction.reply({
                content: '⚠️ Bạn đã mở lá thứ 3 rồi!',
                ephemeral: true
              })
            );
          }
        ...
        // =================================================
        // MA SÓI - PHÙ THỦY ĐỘC SÁT CONFIRM
        // =================================================
