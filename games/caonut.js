...
async function handleGameStart(client, store, gameMsgId, gameData) {
  ...
  for (const userId of playerIds) {
    ...
    if (!userDmData || userDmData.revealed || !userDmData.dmMsg) {
      continue;
    }
...
async function calculateResults(store, client, gameMsgId, gameData, handData) {
  ...
  for (const [userId, hand] of handData) {
    ...
    results.push({
      userId,
      username: player ? player.username : 'Người chơi',
      hand,
      total
    });
  ...
  const resEmbed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: '💰 Pot Tổng',
        value: `${totalPot.toLocaleString()} Mcoin`,
        inline: true
      },
      {
        name: '👑 Người Thắng',
        value: `${winners.length} người`,
        inline: true
      },
      {
        name: '💵 Giải Thưởng/Người',
        value: `${prizePerWinner.toLocaleString()} Mcoin`,
        inline: true
      }
    )
