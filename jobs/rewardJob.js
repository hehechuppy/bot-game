...
function startRewardJob(client) {
  ...
  // Gồm: Bảng xếp hạng Mcoin + Bảng xếp hạng Voice (tiền bằng nhau, voice thêm box)
  new CronJob('0 0 0 * * *', async () => {
    ...
    if (store.economyMap.size > 0) {
      ...
      const mcoinRewards = [300000, 200000, 100000, 36000]; // Bằng với voice leaderboard
    ...
    if (voiceTop10.length > 0) {
      ...
      for (let i = 0; i < voiceTop10.length; i++) {
        ...
        if (rewardData.box > 0) {
          store.addToInventory(uId, 6, rewardData.box); // ID 6 = Lucky Box
        }
    ...
    if (store.backupChannelId && allRewardsLog.length > 0) {
      try {
        ...
        if (channel) {
          // Phân tách thưởng Mcoin và Voice
          const mcoinRewards = allRewardsLog.filter(r => r.type === 'mcoin');
          const voiceRewards = allRewardsLog.filter(r => r.type === 'voice');

          // Thông báo BXH Mcoin (x2)
          if (mcoinRewards.length > 0) {
            ...
            const mcoinEmbed = new EmbedBuilder()
              ...
              .setFooter({ text: 'Tổng thưởng gấp đôi hôm nay!' });

            await channel.send({ embeds: [mcoinEmbed] });
          }

          // Thông báo BXH Voice (x2)
          if (voiceRewards.length > 0) {
            const voiceDesc = voiceRewards.map(r => {
              const boxText = r.box > 0 ? ` + 🎁 ${r.box} Lucky Box` : '';
        ...
        console.error('❌ Lỗi khi gửi thông báo phát thưởng:', e);
      }
    }
